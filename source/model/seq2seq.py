...
class Seq2Seq(nn.Module):
    def __init__(self,src_field,tgt_field,kb_field,kbt_field,embed_size,hidden_size,padding_idx=None,num_layers=1,
                 bidirectional=False,attn_mode="mlp",with_bridge=False,tie_embedding=False,
                 max_dlg_hop=1,max_kb_hop=1,dropout=0.0,use_gpu=False):
        ...
        self.decoder = RNNDecoder(embedder=self.dec_embedder,max_dlg_hop=self.max_dlg_hop,max_kb_hop=self.max_kb_hop,
                                  ...
                                  kbt_output_size=self.kbt_vocab_size,num_layers=self.num_layers,attn_mode=self.attn_mode,memory_size=self.hidden_size,
        ...
        self.src_attention = Attention(max_hop=self.max_dlg_hop,query_size=self.hidden_size,
                                   ...
                                   num_layers=self.num_layers,dropout=self.dropout if self.num_layers > 1 else 0,
    ...
    def dlg_encode(self, enc_inputs, hidden=None):
        ...
        #print(f"ENC hidden: sh: {enc_hidden.shape}")
        # [dir*layers  bs hs]
        #print(f"ENC OP: sh: {enc_outputs.shape}")
        # [bs seq_len hs]
    ...
    def kb_encode(self, kb_tgt_inputs, dlg_enc_hidden, dlg_enc_outputs, dlg_attn_mask, kb_src_inputs, hidden=None):
        kb_src_outputs, kb_src_hidden = self.encoder(kb_src_inputs, hidden)
        kb_tgt_outputs, kb_tgt_hidden = self.encoder(kb_tgt_inputs, hidden)
        outputs = Pack()
    ...
    def collect_metrics(self, outputs, target):
        ...
        _, word_cnt = perplexity(logits, target, padding_idx=self.padding_idx)
        # For ppl
        # metrics.add(loss=loss, acc=acc, vl=(loss,word_cnt))
