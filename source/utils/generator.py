...
class BeamGenerator(object):
    ...
    def forward(self, turn_inputs, enc_hidden=None):
        """
        forward
        """
        ...
        with torch.no_grad():
            ...
            for i, input in enumerate(turn_inputs):
                ...
                dlg_enc_hidden = self.model.dlg_encode(dlg_enc_inputs, enc_hidden)
                enc_outputs, dec_init_state = self.model.kb_encode(kb_enc_inputs, dlg_enc_hidden, enc_hidden)
                preds, lens, scores, dec_state = self.decode(dec_init_state)

                # update memory
                dec_inputs = tgt[:, :-1], tgt_lengths - 1  # filter <eos>
                outputs = self.model.forward(dlg_enc_inputs, kb_enc_inputs, dec_inputs)
                self.model.update_memory(dlg_state_memory=outputs.dlg_state_memory)
                ...
                enc_outputs.add(src=src_text, tgt=tgt_text, preds=preds_text, conv_id=conv_id, turn_label=turn_label, scores=scores)
    ...
    def generate(self, data_iter, output_dir=None, verbos=False):
        ...
        for batch_idx in range(num_batches):
            ...
            result_batch = self.forward(turn_inputs=turn_inputs, enc_hidden=None)
        ...
        for dia, tl, hyp, ref in zip(conv_ids, turn_labels, hyps, refs):
            hyp_str = " ".join(hyp)
            ref_str = " ".join(ref)
